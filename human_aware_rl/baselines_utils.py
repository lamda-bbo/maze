...
from baselines.ppo2.my_ppo2 import my_learn
...
def get_model_policy(step_fn, sim_threads, is_joint_action=False):
    ...
    def state_policy(mdp_state, mdp, agent_index, stochastic=True, return_action_probs=False):
        ...
        a_len = len(action_probs)
        flag = False
        for i in range(a_len):
            if math.isnan(action_probs[i]):
                flag = True
                break
        if flag:
            action_probs = [1.0/a_len for _ in range(a_len)]
...
def update_model(env, model, population=None, ent_version=1, div_version=0, add_ent_to_loss=1, metric_np=None, **kwargs):
    """
...
def my_update_model(env, model, partner=None, agent_population=None, partner_population=None, ent_version=1,
                    div_version=0, add_ent_to_loss=1, metric_np=None, train_experience=None, **kwargs):
    """
    Train agent defined by a model using the specified environment.
    ...
    updated_model, run_info, experience = my_learn(
        network=kwargs["NETWORK_TYPE"],
        env=env,
        total_timesteps=kwargs["PPO_RUN_TOT_TIMESTEPS"],
        nsteps=kwargs["BATCH_SIZE"],
    ...
    return run_info, experience

def overwrite_model(model_from, model_to):
    model_from_vars = tf.trainable_variables(model_from.scope)
    model_from_vars_new = []
    for v in model_from_vars:
        if v.name.split('/')[0] == model_from.scope:
            model_from_vars_new.append(v)

    model_to_vars = tf.trainable_variables(model_to.scope)
    model_to_vars_new = []
    for d in model_to_vars:
        if d.name.split('/')[0] == model_to.scope:
            model_to_vars_new.append(d)
    a = overwrite_variables(model_from_vars_new, model_to_vars_new)
    return a

def overwrite_variables(variables_to_copy, variables_to_overwrite):
    sess = tf.get_default_session()
    restores = []
    if len(variables_to_copy) == len(variables_to_overwrite):
