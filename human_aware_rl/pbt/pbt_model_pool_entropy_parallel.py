...
sys.path.append('/home/mars/wangyt/mep/human_aware_rl/')
...
class PBTAgent(object):
    ...
    def update(self, gym_env, population=None, ent_version=1, div_version=0, add_ent_to_loss=1):
        with tf.device('/device:GPU:{}'.format(self.params["GPU_ID"])):
            train_info = update_model(gym_env, self.model, population, ent_version, div_version=div_version,
                                      add_ent_to_loss=add_ent_to_loss, **self.params)
...
def my_config():
    ...
    if TIMESTAMP_DIR:
        SAVE_DIR = PBT_DATA_DIR + time.strftime('%Y_%m_%d-%H_%M_%S_') + EX_NAME + '_div'+ str(DIV_VERSION) + '_entloss' + str(ADD_ENT_TO_LOSS) + "/"
    else:
    ...
    TOTAL_STEPS_PER_AGENT = 2e7 if not LOCAL_TESTING else 1e4
...
def pbt_one_run(params, seed):
    ...
    save_dict_to_file(params, params["SAVE_DIR"] + "config") # 'PBT_DATA_DIR/pbt_simple/seed_9015/config'
    ...
    # pbt #
    #######
    # we can get the start_position from the grid
    ...
    def pbt_training():
        ...
        for pbt_iter in range(1, params["NUM_PBT_ITER"] + 1):
            ...
            print(f"*********************Check memory: {pbt_iter}_iteration start *******************")
            ...
            for sel_iter in range(params["ITER_PER_SELECTION"]):
                ...
                print(f"*********************Check memory: before update agent {pbt_iter}_{upd_num} *******************")
                print(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024)

                pbt_agent1.update(gym_env, pbt_population_copy, ent_version=params['ENT_VERSION'],
                                  div_version=params['DIV_VERSION'], add_ent_to_loss=params['ADD_ENT_TO_LOSS'])

                print(f"*********************Check memory: after update agent {pbt_iter}_{upd_num} *******************")
                print(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024)
            ...
            avg_ep_returns_dense_dict = defaultdict(list)
            ...
            for i, pbt_agent in enumerate(pbt_population):
                ...
                avg_ep_returns_dense_dict[i].append(dense_rews)
            ...
            for k in range(population_size):
                # logz.log_tabular(f"Agent{k}SparseReward", avg_ep_returns_sparse_dict[k][0])
                logz.log_tabular(f"Agent{k}DenseReward", format(np.mean(avg_ep_returns_dense_dict[k][0]), '.5f'))
                # logz.log_tabular(f"Agent{k}DenseReward", avg_ep_returns_dense_dict[k][0])
                # logz.log_tabular(f"Agent{k}DenseRewarAvg", np.mean(avg_ep_returns_dense_dict[k][0]))
            for k in range(population_size):
                # logz.log_tabular(f"Agent{k}SparseReward", avg_ep_returns_sparse_dict[k][0])
                logz.log_tabular(f"Agent{k}SparseReward", format(np.mean(avg_ep_returns_sparse_dict[k][0]), '.5f'))
                # logz.log_tabular(f"Agent{k}DenseReward", avg_ep_returns_dense_dict[k][0])
                # logz.log_tabular(f"Agent{k}DenseRewarAvg", np.mean(avg_ep_returns_dense_dict[k][0]))
            # logz.log_tabular("AverageDenseReward",
            #                  np.mean([np.mean(avg_ep_returns_dense_dict[i][0]) for i in range(population_size)]))
            logz.log_tabular("MinSparseReward",
                             format(np.min([np.mean(avg_ep_returns_sparse_dict[i][0]) for i in range(population_size)]),'.5f'))
            logz.log_tabular("MaxSparseReward",
                             format(np.max([np.mean(avg_ep_returns_sparse_dict[i][0]) for i in range(population_size)]), '.5f'))
            logz.log_tabular("AverageSparseReward",
                             format(np.mean([np.mean(avg_ep_returns_sparse_dict[i][0]) for i in range(population_size)]), '.5f'))
            logz.log_tabular("StdSparseReward",
                             format(np.std([np.mean(avg_ep_returns_sparse_dict[i][0]) for i in range(population_size)]),'.5f'))
            logz.log_tabular("Memory", format(me,'.5f'))
            logz.dump_tabular()
