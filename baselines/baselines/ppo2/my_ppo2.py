...
try:
    ...
    MPI = None
from baselines.ppo2.my_runner import Runner
...
def my_learn(*, network, env, total_timesteps, early_stopping = False, eval_env = None, seed=None, nsteps=2048,
          ent_coef=0.0, ent_pool_coef=0.0, ent_version=1, div_version=0, add_ent_to_loss=1, lr=3e-4, vf_coef=0.5,
          max_grad_norm=0.5, gamma=0.99, lam=0.95, log_interval=10, nminibatches=4, noptepochs=4, cliprange=0.2,
          save_interval=0, load_path=None, model_fn=None, partner=None, agent_population=None, partner_population=None,
          metric_np=None, train_experience=None, scope='', **network_kwargs):
    '''
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    if model_fn is None:
        from baselines.ppo2.my_model import Model
        model_fn = Model

    model = model_fn(policy=policy, ob_space=ob_space, ac_space=ac_space, nbatch_act=nenvs, nbatch_train=nbatch_train,
                    nsteps=nsteps, ent_coef=ent_coef, vf_coef=vf_coef,
                    max_grad_norm=max_grad_norm, scope=scope, load_path=load_path, add_ent_to_loss=add_ent_to_loss)

    if load_path is not None:
        model.load(load_path)
    # Instantiate the runner object
    runner = Runner(env=env, model=model, nsteps=nsteps, gamma=gamma, lam=lam, partner=partner, agent_population=agent_population,
                    partner_population=partner_population, ent_pool_coef=ent_pool_coef, ent_version=ent_version, div_version=div_version)
    ...
    for update in range(1, nupdates+1):
        ...
        if train_experience is None:
            results_1, results_2 = runner.run()
            obs, returns, masks, actions, values, neglogpacs, states, epinfos = results_1
            obs_partner, returns_partner, masks_partner, actions_partner, values_partner, neglogpacs_partner, states_partner = results_2 #pylint: disable=E0632
            epinfos_partner = epinfos.copy()
            experience.append([obs_partner, returns_partner, masks_partner, actions_partner, values_partner, neglogpacs_partner, states_partner, epinfos_partner])
        else:
            obs, returns, masks, actions, values, neglogpacs, states, epinfos = train_experience[update-1]
        ...
        if update % log_interval == 0 or update == 1:
            ...
            # print(env.venv.remote_get_agent_idx())
    ...
    return model, run_info, experience
